...
const Event = ({ color, title, url = "", image = "" }: Props) => {
  return (
    <div className="grid grid-cols-2 w-[400px]">
      ...
      <ColorBox title={title} color={color} bonus_classes="hover:scale-105 "></ColorBox>
      <button
        className={
          //changes class names and content if url is blank
          "my-1 text-center w-90% h-[100%] small-text-bg col-span-2 " + (url == "" && "opacity-50 hover:opacity-50 ")
        }
        onClick={() => {
    ...
    <RevealOnScroll>
      ...
      <div className="flex justify-center items-center mt-10">
        <p className="m-5 small-text-bg p-2 lg:w-[50%] w-[80%]">
          These are events in which I participated involving making some sort of product, whether it be making a game,
          coding, or frontend development. These events have taken place in jobs, hackathons, online classes (i.e.
          Udemy) or just general challenges I pursued to brighten my skills.
        </p>
      </div>
      <div className="justify-center items-center flex mt-10  animate-phase-in">
        <div className="responsive-grid place-items-center w-3/4 p-2">
          ...
          />
          <Event color="grey" title="Power BI Udemy Course" url="/#/pbi/" image="/events/power-bi.png" />
          <Event color="blue" title="Python ML Udemy Course" url="/#/python_ml/" image="/events/python_ml.png" />
          <Event
            color="teal"
          ...
          <Event
            color="indigo"
            title="DeltaHacks XII - Olympiknights!"
            url="/#/deltahacksxii/"
            image="/events/deltahacksxii.png"
          />
        </div>
      </div>
    </RevealOnScroll>
